...
from admins import is_admin, add_admin, get_admins_list
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        webapp_url = f"https://{os.getenv('REPL_SLUG', 'test4-01hx')}.onrender.com/telegram-auth?telegram_id={user_id}"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "add_admin":
        ...
        # Create inline keyboard with Web App button  
        import os
        webapp_url = f"https://workspace.onrender.com/telegram-auth?telegram_id={user_id}"
        logger.info(f"Generated webapp URL for user {user_id}: {webapp_url}")
        keyboard = [
            [InlineKeyboardButton(
                "🚀 Открыть веб-приложение", 
...
def main() -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if bot_info.get('ok'):
                logger.info(f"Bot token is valid. Bot name: {bot_info['result']['first_name']}")
            else:
                logger.error(f"Bot token test failed: {bot_info}")
                return
        else:
            logger.error(f"Bot token test failed with status code: {response.status_code}")
            return
    except Exception as e:
